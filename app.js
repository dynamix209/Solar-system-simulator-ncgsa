...
const PLANETS = [
  { key:'mercury', relMass:0.0553, name:'Mercury', type:'Terrestrial planet', color:0x9c9186, emissive:0x1a1712,
    diameterKm:4879, periodDays:87.97, periodLabel:'88 days', distanceAU:0.387, displayRadius:0.50, moonCount:0,
  ...
  { key:'venus', relMass:0.815, name:'Venus', type:'Terrestrial planet', color:0xe8c78a, emissive:0x2a2114,
    ...
    diameterKm:12104, periodDays:224.70, periodLabel:'225 days', distanceAU:0.723, displayRadius:1.14, moonCount:0,
  ...
  { key:'earth', relMass:1.0, name:'Earth', type:'Terrestrial planet', color:0x3d7ab5, emissive:0x0a1520,
    diameterKm:12742, periodDays:365.256, periodLabel:'365.26 days (1 year)', distanceAU:1.000, displayRadius:1.20, moonCount:1,
  ...
  { key:'mars', relMass:0.107, name:'Mars', type:'Terrestrial planet', color:0xb1502f, emissive:0x1f0d05,
    diameterKm:6779, periodDays:686.98, periodLabel:'687 days (1.88 years)', distanceAU:1.524, displayRadius:0.68, moonCount:2,
  ...
  { key:'jupiter', relMass:317.8, name:'Jupiter', type:'Gas giant', color:0xd8b48c, emissive:0x241c12,
    diameterKm:139820, periodDays:4332.59, periodLabel:'11.86 years', distanceAU:5.203, displayRadius:5.50, moonCount:95,
  ...
  { key:'saturn', relMass:95.2, name:'Saturn', type:'Gas giant', color:0xe4d0a0, emissive:0x241f14,
    diameterKm:116460, periodDays:10759.22, periodLabel:'29.46 years', distanceAU:9.537, displayRadius:4.58, moonCount:146,
  ...
  { key:'uranus', relMass:14.5, name:'Uranus', type:'Ice giant', color:0xb8e8e0, emissive:0x0a1c1c,
    diameterKm:50724, periodDays:30688.5, periodLabel:'84.01 years', distanceAU:19.191, displayRadius:2.00, moonCount:28,
  ...
  { key:'neptune', relMass:17.1, name:'Neptune', type:'Ice giant', color:0x3a6fd0, emissive:0x061018,
    diameterKm:49244, periodDays:60182, periodLabel:'164.8 years', distanceAU:30.07, displayRadius:1.94, moonCount:16,
...
let playing = true; // simulation runs on load
let daysPerSecond = 1/86400; // overwritten by applySimSpeed() from slider default
let simulatedDate = new Date();
(function seedSimDateLabel(){
  try {
    const el = document.getElementById('simdate-value');
    if(!el) return;
    const d = simulatedDate;
    el.textContent = d.getUTCFullYear() + '-' + String(d.getUTCMonth()+1).padStart(2,'0') + '-' + String(d.getUTCDate()).padStart(2,'0') + '  ' + String(d.getUTCHours()).padStart(2,'0') + ':' + String(d.getUTCMinutes()).padStart(2,'0') + ' UTC';
  } catch(e){}
})();
...
if(!renderer){
  ...
  box.innerHTML = '<div style="max-width:460px"><h2 style="color:#c9a227;margin:0 0 12px">WebGL unavailable</h2>' +
    '<p style="line-height:1.55;color:#9aa3b0">This browser could not create a WebGL context.</p>' +
    '<p style="line-height:1.55;color:#9aa3b0;font-size:14px;text-align:left;margin:12px auto;max-width:400px">1. Open in <b>Microsoft Edge</b> or Chrome (normal tab)<br>2. Settings → System → enable <b>hardware acceleration</b> → restart<br>3. Visit <code style="color:#c9a227">edge://gpu</code> — WebGL must not be Disabled<br>4. Close other 3D tabs → Reload</p>' +
  ...
  window.__NCGSA_NO_WEBGL = true;
  WEBGL_OK = false;
  console.error('[NCGSA] WebGL unavailable');
}
let maxAnisotropy = 4;
let maxTextureSize = 4096;
let SUPPORTS_8K = false;
if(renderer && WEBGL_OK){
  ...
  maxAnisotropy = (renderer.capabilities && renderer.capabilities.getMaxAnisotropy) ? renderer.capabilities.getMaxAnisotropy() : 4;
  ...
  SUPPORTS_8K = maxTextureSize >= 8192;
...
const TEXTURE_URLS = {
  ...
  venus:      TEX + '2k_venus_atmosphere.jpg',
  ...
  // Verified jsDelivr SSS maps (200 OK) for Mars / Jupiter / Saturn
...
function makeGasGiantTexture(kind){
  const w=2048,h=1024;
  const cnv=document.createElement('canvas'); cnv.width=w; cnv.height=h;
  const ctx=cnv.getContext('2d');
  let bands;
  if(kind==='jupiter'){
    bands=[[0,'#e8dcc0'],[0.12,'#c9a070'],[0.22,'#f0e6d0'],[0.32,'#b08050'],[0.42,'#e8d8b8'],[0.52,'#c89860'],[0.62,'#f0e8d0'],[0.72,'#a87848'],[0.82,'#e0d0b0'],[1,'#c8b090']];
  } else if(kind==='saturn'){
    bands=[[0,'#f0e8d0'],[0.2,'#e0d4b0'],[0.4,'#d4c498'],[0.6,'#e8dcc0'],[0.8,'#d0c090'],[1,'#e4d8b8']];
  } else if(kind==='uranus'){
    bands=[[0,'#c8e8e8'],[0.25,'#a8d8d8'],[0.5,'#b8e0e0'],[0.75,'#98d0d0'],[1,'#d0ecec']];
  } else {
    bands=[[0,'#6a8fd0'],[0.25,'#4a70b8'],[0.5,'#5c82c8'],[0.75,'#3d62a8'],[1,'#7090d8']];
  }
  for(let i=0;i<bands.length-1;i++){
    const y0=bands[i][0]*h,y1=bands[i+1][0]*h;
    const g=ctx.createLinearGradient(0,y0,0,y1);
    g.addColorStop(0,bands[i][1]); g.addColorStop(1,bands[i+1][1]);
    ctx.fillStyle=g; ctx.fillRect(0,y0,w,Math.ceil(y1-y0)+1);
  }
  if(kind==='jupiter'){
    const rg=ctx.createRadialGradient(w*0.62,h*0.62,2,w*0.62,h*0.62,h*0.09);
    rg.addColorStop(0,'rgba(180,70,40,0.75)'); rg.addColorStop(1,'rgba(140,50,30,0)');
    ctx.fillStyle=rg; ctx.beginPath(); ctx.ellipse(w*0.62,h*0.62,w*0.07,h*0.05,0.2,0,Math.PI*2); ctx.fill();
  }
  for(let i=0;i<4000;i++){
...
const sunGeo = new THREE.SphereGeometry(7.2, 96, 96);
const sunMat = new THREE.MeshBasicMaterial({ map: sunTexA, color: 0xffffff });
const sunMesh = new THREE.Mesh(sunGeo, sunMat);
sunGroup.add(sunMesh);
const sunGeo2 = new THREE.SphereGeometry(7.35, 96, 96);
...
sunSpiralSprite.scale.set(16,16,1);
...
PLANETS.forEach(p => {
  ...
  const sphereSegs = isTerrestrial ? 176 : 64;
  // Show correct look immediately (bands for giants), then upgrade to photo maps from HTML textures
  let starterMap = null;
  if(p.key === 'jupiter' || p.key === 'saturn' || p.key === 'uranus' || p.key === 'neptune'){
    if(typeof makeGasGiantTexture === 'function') starterMap = makeGasGiantTexture(p.key);
  }
  const tex = loadColorTexture(p.key, function(loadedTex){
    if(!loadedTex || !loadedTex.image || !loadedTex.image.width) return;
    mat.map = loadedTex;
    ...
    if(isTerrestrial && p.key !== 'earth'){
      ...
      if(dispTex){
        ...
        mat.displacementScale = p.displayRadius * 0.05;
        mat.displacementBias = -p.displayRadius * 0.025;
        mat.needsUpdate = true;
      }
    }
  }, p.color);
  if(starterMap && starterMap.image){
    tex.image = starterMap.image;
    tex.needsUpdate = true;
  }
  ...
  if(['jupiter','saturn','uranus','neptune'].includes(p.key)){
    const cols = { jupiter:0xc8b090, saturn:0xe0d0a0, uranus:0xb0e8e4, neptune:0x3a70d8 };
    const atmo = new THREE.Mesh(
...
const MOONS = [
  ...
  { parent:'earth', name:'Moon', radius:0.33, dist:2.55, periodDays:27.322, periodLabel:'27.3 days', diameterKm:3474, color:0xc8c4bc,
  ...
  { parent:'mars', name:'Phobos', radius:0.030, dist:0.95, periodDays:0.3189, periodLabel:'7.66 hours', diameterKm:22.5, color:0x9a8a78,
  ...
  { parent:'mars', name:'Deimos', radius:0.022, dist:1.35, periodDays:1.263, periodLabel:'1.26 days', diameterKm:12.4, color:0x8a7a6a,
    discovered:'1877', discoverer:'Asaph Hall (USNO)',
  ...
  { parent:'jupiter', name:'Sinope', radius:0.024, dist:18.5, periodDays:758.9, aKm:23939000, periodLabel:'758.9 days', diameterKm:35, color:0x6a6050, retrograde:true, e:0.25, iDeg:158,
  ...
  { parent:'saturn', name:'Mimas', radius:0.038, dist:11.15, periodDays:0.942, periodLabel:'22.6 hours', diameterKm:396, color:0xd0d0d0,
  ...
  { parent:'saturn', name:'Enceladus', radius:0.042, dist:11.55, periodDays:1.370, periodLabel:'1.37 days', diameterKm:504, color:0xf0f4f8,
  ...
  { parent:'saturn', name:'Tethys', radius:0.048, dist:11.95, periodDays:1.888, periodLabel:'1.89 days', diameterKm:1062, color:0xe8e8e0,
  ...
  { parent:'saturn', name:'Dione', radius:0.048, dist:12.4, periodDays:2.737, periodLabel:'2.74 days', diameterKm:1123, color:0xd8d4d0,
    discovered:'1684', discoverer:'Giovanni Domenico Cassini', fact:'Bright ice cliffs (\"wispy terrain\") on the trailing hemisphere.' },
  { parent:'saturn', name:'Rhea', radius:0.055, dist:13.1, periodDays:4.518, periodLabel:'4.52 days', diameterKm:1528, color:0xc8c4c0,
    discovered:'1672', discoverer:'Giovanni Domenico Cassini', fact:'Second-largest moon of Saturn; heavily cratered ice world.' },
  { parent:'saturn', name:'Titan', radius:0.14, dist:14.8, periodDays:15.945, periodLabel:'15.9 days', diameterKm:5150, color:0xd4a86a,
  ...
  { parent:'saturn', name:'Iapetus', radius:0.055, dist:16.8, periodDays:79.33, periodLabel:'79.3 days', diameterKm:1470, color:0x8a7a68,
  ...
  // Uranus
  { parent:'uranus', name:'Miranda', radius:0.04, dist:2.45, periodDays:1.413, periodLabel:'1.41 days', diameterKm:472, color:0xc0c8d0,
  ...
  { parent:'uranus', name:'Ariel', radius:0.05, dist:2.85, periodDays:2.520, periodLabel:'2.52 days', diameterKm:1158, color:0xb8c0c8,
    discovered:'1851', discoverer:'William Lassell', fact:'Brightest major Uranian moon; young surfaces and rift valleys.' },
  { parent:'uranus', name:'Umbriel', radius:0.05, dist:3.25, periodDays:4.144, periodLabel:'4.14 days', diameterKm:1169, color:0x707880,
    discovered:'1851', discoverer:'William Lassell', fact:'Darkest major Uranian moon; ancient cratered surface.' },
  { parent:'uranus', name:'Titania', radius:0.065, dist:3.75, periodDays:8.706, periodLabel:'8.7 days', diameterKm:1578, color:0xb0c0c8,
    discovered:'1787', discoverer:'William Herschel', fact:'Largest moon of Uranus; huge fault systems from past expansion.' },
  { parent:'uranus', name:'Oberon', radius:0.06, dist:4.25, periodDays:13.46, periodLabel:'13.5 days', diameterKm:1523, color:0x9098a0,
    discovered:'1787', discoverer:'William Herschel', fact:'Outermost major Uranian moon; old, cratered ice-rock mix.' },
  // Neptune
  { parent:'neptune', name:'Proteus', radius:0.04, dist:2.35, periodDays:1.122, periodLabel:'1.12 days', diameterKm:420, color:0x6a7078,
  ...
  { parent:'neptune', name:'Triton', radius:0.10, dist:3.15, periodDays:5.877, periodLabel:'5.88 days', diameterKm:2707, color:0x90a0b0, retrograde:true, iDeg:156.8, e:0.000016,
  ...
  { parent:'neptune', name:'Nereid', radius:0.035, dist:4.0, periodDays:360.1, periodLabel:'360 days', diameterKm:340, color:0x808890,
...
(function addMoreMoons(){
  ...
  jupOuter.forEach(function(o, i){
    MOONS.push({
      ...
      dist: 16.5 + i*0.04, // provisional; applyJupiterLinearDistances / rescale will compress
      periodDays: o.p, periodLabel: o.p + ' days',
      diameterKm: Math.max(1, o.d), color:0x7a7068,
      aKm: 11000000 + i * 400000, // synthetic a for log compression
      discovered: String(o.y), discoverer: o.by,
      fact: 'Irregular outer moon of Jupiter (NASA/JPL). Display orbit log-compressed for visibility.'
    });
  });
  ...
  jupExtra.forEach(function(name, i){
    MOONS.push({
      ...
      aKm: 15000000 + i * 350000,
      discovered: '2003–2022 surveys', discoverer: 'Sheppard / Gladman et al.',
      fact: 'Small irregular outer moon of Jupiter (IAU / NASA). Display orbit compressed.'
    });
  });
  const satOuter = [
  ...
  satOuter.forEach(function(name, i){
    // Pack outer Saturn moons in a tight shell beyond Titan (display ~R*4.2 .. R*6.5)
    const satR = 4.6;
    const t = i / Math.max(1, satOuter.length - 1);
    MOONS.push({
      parent:'saturn', name:name, radius:0.011+((i%4)*0.0015),
      dist: satR * 4.2 + t * (satR * 2.2),
      ...
      fact: 'Named or provisional moon of Saturn (NASA/JPL). Orbit outside the rings (display-compressed).'
    });
  });
  ...
  uraOuter.forEach(function(name, i){
    const uR = 2.8;
    const t = i / Math.max(1, uraOuter.length - 1);
    MOONS.push({
      parent:'uranus', name:name, radius:0.016, dist: uR * 2.15 + t * (uR * 1.6),
      ...
      fact: 'Moon of Uranus (NASA/JPL). Display orbit compressed.'
    });
  });
  ...
  nepOuter.forEach(function(name, i){
    const nR = 2.7;
    const t = i / Math.max(1, nepOuter.length - 1);
    MOONS.push({
      parent:'neptune', name:name, radius:0.014, dist: nR * 1.85 + t * (nR * 1.5),
      ...
      fact: 'Moon of Neptune (NASA/JPL). Display orbit compressed.'
    });
  });
})();
...
function makeMoonTexture(hexColor){
  // Higher-detail procedural regolith: limb darkening, multi-scale fBm noise,
  // power-law craters with rims/ejecta, mare basins, micro-grain.
  const w=1024, h=512;
  const cnv=document.createElement('canvas'); cnv.width=w; cnv.height=h;
  const ctx=cnv.getContext('2d');
  const r=(hexColor>>16)&255, g=(hexColor>>8)&255, b=hexColor&255;
  // Base polar gradient (limb darkening proxy)
  const grad = ctx.createLinearGradient(0, 0, 0, h);
  grad.addColorStop(0, 'rgb('+Math.max(0,r-38)+','+Math.max(0,g-34)+','+Math.max(0,b-28)+')');
  grad.addColorStop(0.22, 'rgb('+Math.max(0,r-12)+','+Math.max(0,g-10)+','+Math.max(0,b-8)+')');
  grad.addColorStop(0.5, 'rgb('+r+','+g+','+b+')');
  grad.addColorStop(0.78, 'rgb('+Math.max(0,r-12)+','+Math.max(0,g-10)+','+Math.max(0,b-8)+')');
  grad.addColorStop(1, 'rgb('+Math.max(0,r-38)+','+Math.max(0,g-34)+','+Math.max(0,b-28)+')');
  ctx.fillStyle = grad; ctx.fillRect(0,0,w,h);
  // Large-scale albedo noise (highlands vs lowlands)
  function hash2(x,y){ const n=Math.sin(x*127.1+y*311.7)*43758.5453; return n-Math.floor(n); }
  function noise2(x,y){
    const xi=Math.floor(x), yi=Math.floor(y), xf=x-xi, yf=y-yi;
    const u=xf*xf*(3-2*xf), v=yf*yf*(3-2*yf);
    const a=hash2(xi,yi), c=hash2(xi+1,yi), d=hash2(xi,yi+1), e=hash2(xi+1,yi+1);
    return a+(c-a)*u+(d-a)*v+(a-c-d+e)*u*v;
  }
  function fbm(x,y,oct){ let s=0,a=0.5,f=1,n=0; for(let i=0;i<oct;i++){ s+=a*noise2(x*f,y*f); n+=a; a*=0.5; f*=2.05; } return s/n; }
  const img = ctx.getImageData(0,0,w,h);
  const d = img.data;
  for(let y=0;y<h;y++){
    for(let x=0;x<w;x++){
      const n = fbm(x*0.008, y*0.016, 5);
      const n2 = fbm(x*0.03+9, y*0.06, 3);
      const i = (y*w+x)*4;
      const k = 0.72 + 0.38*n + 0.12*n2;
  ...
  // Mare / dark plains
  for(let i=0;i<18;i++){
    const x=Math.random()*w, y=Math.random()*h, rad=28+Math.random()*85;
    const grd=ctx.createRadialGradient(x,y,0,x,y,rad);
    grd.addColorStop(0,'rgba('+(r*0.42|0)+','+(g*0.42|0)+','+(b*0.4|0)+',0.55)');
    grd.addColorStop(0.7,'rgba('+(r*0.5|0)+','+(g*0.48|0)+','+(b*0.45|0)+',0.28)');
    grd.addColorStop(1,'rgba(0,0,0,0)');
    ctx.beginPath(); ctx.ellipse(x,y,rad*1.5,rad*0.72,Math.random()*Math.PI,0,Math.PI*2);
    ctx.fillStyle=grd; ctx.fill();
  }
  // Procedural crater field
  if(typeof stampCraterField === 'function'){
    stampCraterField(ctx, w, h, 55, 160);
  } else {
    for(let i=0;i<220;i++){
      const x=Math.random()*w, y=Math.random()*h, rad=1.2+Math.random()*18;
      const shade=0.4+Math.random()*0.45;
      ctx.beginPath(); ctx.arc(x,y,rad,0,Math.PI*2);
      ctx.fillStyle='rgba('+(r*shade|0)+','+(g*shade|0)+','+(b*shade|0)+',0.78)';
      ctx.fill();
      if(rad>6){
        ctx.beginPath(); ctx.arc(x,y,rad*1.08,0,Math.PI*2);
        ctx.strokeStyle='rgba(230,225,215,0.22)'; ctx.lineWidth=1.2; ctx.stroke();
      }
    }
  }
  // Micro-grain / regolith sparkle
  for(let i=0;i<520;i++){
    ctx.fillStyle='rgba(255,255,255,'+(0.02+Math.random()*0.09)+')';
    ctx.fillRect(Math.random()*w, Math.random()*h, 1+Math.random()*3, 1);
  }
  for(let i=0;i<280;i++){
...
function makeTypedMoonMap(hexColor, hint){
  ...
  // Extra visual cues by surface class
  try {
    ...
    if(img && img.getContext){
      ...
      if(hint === 'volcanic'){
        // Io-like: sulfur plains + dark calderas + bright hotspots
        ctx.fillStyle = 'rgba(210,140,40,0.18)';
        ctx.fillRect(0,0,W,H);
        for(let i=0;i<55;i++){
          const x=Math.random()*W, y=Math.random()*H, r=3+Math.random()*14;
          const g=ctx.createRadialGradient(x,y,0,x,y,r);
          g.addColorStop(0,'rgba(255,90,20,0.55)');
          g.addColorStop(0.5,'rgba(180,40,10,0.3)');
          g.addColorStop(1,'rgba(0,0,0,0)');
          ctx.beginPath(); ctx.arc(x,y,r,0,7); ctx.fillStyle=g; ctx.fill();
        }
        for(let i=0;i<25;i++){
          ctx.fillStyle='rgba(30,20,15,0.45)';
          ctx.beginPath(); ctx.arc(Math.random()*W, Math.random()*H, 4+Math.random()*10, 0, 7); ctx.fill();
        }
      } else if(hint === 'ice'){
        // Bright icy crust + linear fractures (Europa-like) + soft blue tint
        ctx.fillStyle = 'rgba(200,220,245,0.28)';
        ctx.fillRect(0,0,W,H);
        for(let i=0;i<110;i++){
          const x=Math.random()*W, y=Math.random()*H;
          ctx.strokeStyle = 'rgba(255,255,255,'+(0.1+Math.random()*0.25)+')';
          ctx.lineWidth = 0.8+Math.random()*2.2;
          ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x+(30+Math.random()*50)*(Math.random()>0.5?1:-1), y+Math.random()*10-5); ctx.stroke();
        }
        for(let i=0;i<70;i++){
          const x=Math.random()*W, y=Math.random()*H, r=2+Math.random()*14;
          ctx.beginPath(); ctx.arc(x,y,r,0,Math.PI*2);
          ctx.fillStyle='rgba(35,45,65,0.28)'; ctx.fill();
        }
      } else if(hint === 'haze'){
        // Titan: orange organic haze, smooth with soft bands
        const hg = ctx.createLinearGradient(0,0,0,H);
        hg.addColorStop(0,'rgba(180,120,50,0.35)');
        hg.addColorStop(0.5,'rgba(200,150,70,0.28)');
        hg.addColorStop(1,'rgba(160,100,40,0.35)');
        ctx.fillStyle = hg; ctx.fillRect(0,0,W,H);
        for(let i=0;i<8;i++){
          ctx.fillStyle='rgba(255,200,120,0.06)';
          ctx.fillRect(0, H*(i/8), W, H*0.04);
        }
...
(function applyJupiterLinearDistances(){
  ...
  const R = jup ? jup.displayRadius : 5.4;
  // Galileans: linear scale so Callisto sits at ~2.9 R (readable, ordered)
  const callistoTarget = R * 2.9;
  ...
  // Outer irregulars (beyond ~50 Rj): log-compress into a tight shell past Callisto
  // so Sinope etc. stay near Jupiter, not halfway across the solar system view.
  const outerShellMin = callistoTarget + 0.6;
  const outerShellMax = R * 5.8;
  MOONS.forEach(function(m){
    if(m.parent !== 'jupiter') return;
    const aKm = m.aKm || table[m.name];
    if(aKm){
      const aRj = aKm / JUP_R_KM;
      if(aRj <= (CALLISTO_A / JUP_R_KM) * 1.05){
        m.dist = Math.max(R + (m.radius||0.02) + 0.55, aRj * compress);
      } else {
        // log map real aRj from ~50–350 Rj → outerShellMin..outerShellMax
        const t = Math.log(Math.max(aRj, 30) / 30) / Math.log(350 / 30);
        m.dist = outerShellMin + Math.min(1, Math.max(0, t)) * (outerShellMax - outerShellMin);
      }
    } else {
      // provisional outer moons: pack just outside Callisto shell by index order
      m.dist = Math.max(outerShellMin + 0.15, Math.min(outerShellMax, m.dist || (outerShellMin + 0.4)));
    }
  });
})();
...
(function rescaleMoonDistances(){
  // First pass: enforce min distance from planet / rings
  MOONS.forEach(function(m){
    ...
    const minDist = Math.max(R + (m.radius || 0.05) + 0.55, R * ringF + 0.25);
    if(!(m.dist > minDist)) m.dist = minDist + (m.periodDays ? Math.min(3, m.periodDays * 0.02) : 0.35);
    if(parent.key === 'earth' && m.name === 'Moon') m.dist = Math.max(R + 1.8, m.dist);
    if(parent.key === 'saturn'){
      const satMin = R * 2.45 + (m.radius || 0.05);
      if(m.dist < satMin) m.dist = satMin + (m.name === 'Titan' ? 1.8 : Math.min(2.5, (m.periodDays || 5) * 0.05));
    }
    // Cap outer irregulars so they never fly across the whole orrery
    if(parent.key === 'jupiter' && m.dist > R * 6.0) m.dist = R * 6.0;
    if(parent.key === 'saturn' && m.dist > R * 6.5) m.dist = R * 6.5;
    if(parent.key === 'uranus' && m.dist > R * 4.2) m.dist = R * 4.2;
    if(parent.key === 'neptune' && m.dist > R * 4.0) m.dist = R * 4.0;
  });
  // Second pass: per-parent radial separation so moon spheres never interpenetrate
  const byParent = {};
  MOONS.forEach(function(m){
  ...
  Object.keys(byParent).forEach(function(pk){
    const list = byParent[pk].slice().sort(function(a,b){ return (a.dist||0) - (b.dist||0); });
    for(let i=1;i<list.length;i++){
      const prev = list[i-1], cur = list[i];
      const gap = (prev.radius||0.02) + (cur.radius||0.02) + 0.08;
      if(cur.dist < prev.dist + gap) cur.dist = prev.dist + gap;
    }
  });
})();
...
MOONS.forEach(m => {
  ...
  if(m.name === 'Moon'){
    ...
    moonMap = (typeof makeTypedMoonMap === 'function')
      ? makeTypedMoonMap(m.color || 0x999999, hint || 'rocky')
      : makeMoonTexture(m.color || 0x999999);
  }
  if(!moonMap) moonMap = makeMoonTexture(m.color || 0x999999);
  ...
  if(!moonMap && typeof makeMoonTexture === 'function') moonMap = makeMoonTexture(m.color || 0xaaaaaa);
  ...
  // Orbit paths for major moons + a few notable irregulars (Keplerian ellipse)
  const ORBIT_LINE_MOONS = {
    Moon:1, Phobos:1, Deimos:1,
    Io:1, Europa:1, Ganymede:1, Callisto:1, Himalia:1, Sinope:1,
    Mimas:1, Enceladus:1, Tethys:1, Dione:1, Rhea:1, Titan:1, Iapetus:1,
    Miranda:1, Ariel:1, Umbriel:1, Titania:1, Oberon:1,
    Proteus:1, Triton:1, Nereid:1
  };
  if(ORBIT_LINE_MOONS[m.name] || (m.radius >= 0.05 && m.parent !== 'jupiter')){
    const e0 = (m.e != null) ? Math.min(0.35, m.e) : 0.012;
    const i0 = Math.min(18, Math.abs(m.iDeg != null ? m.iDeg : 1)) * 0.14;
    const node0 = (m.angle0 || 0) * 180 / Math.PI;
    const peri0 = node0 + 20;
    const segsO = 96;
    const opts = [];
    for(let i=0;i<=segsO;i++){
      const Md = (i/segsO)*360;
      const s = keplerianXYZ(m.dist, e0, i0, node0, peri0, Md);
      opts.push(new THREE.Vector3(s.x, s.z, s.y));
    }
    const oGeo = new THREE.BufferGeometry().setFromPoints(opts);
    const oMat = new THREE.LineBasicMaterial({
      color: 0xc9a227, transparent:true, opacity:0.32, depthWrite:false
    });
...
function updateMoons(simDays){
  ...
  // Laplace resonance lock: Io:Europa:Ganymede mean-motion ≈ 4:2:1
  const laplacePhase = { Io: 0, Europa: Math.PI * 0.5, Ganymede: Math.PI };
  // Known retrograde outer moons
  const RETRO = { Triton:1, Phoebe:1, Pasiphae:1, Sinope:1, Carme:1, Ananke:1, Nereid:0 };

  MOONS.forEach(function(m){
    if(!m.mesh || !m.pos) return;
    // Real eccentricity when known; mild default so orbits are slightly elliptical (Kepler 1)
    let e = (m.e != null) ? m.e : 0.012;
    e = Math.min(0.35, Math.max(0, e));
    // Inclination: use published iDeg; scale for readability but keep relative hierarchy
    let iDeg = (m.iDeg != null) ? m.iDeg : ((m.inc0 || 0.02) * 180 / Math.PI);
    if(RETRO[m.name] || m.retrograde) m.retrograde = true;
    const node = (m.angle0 || 0) * 180 / Math.PI;
    const peri = node + (m.w0 != null ? m.w0 : 18 + (m.name ? m.name.length * 3 : 0) % 40);
    // Mean anomaly advances uniformly with time → equal areas (Kepler 2)
    const period = Math.max(0.01, m.periodDays || 10);
    let Mdeg = ((simDays / period) * 360 + (m.angle0 || 0) * 180 / Math.PI) % 360;
    if(Mdeg < 0) Mdeg += 360;
    ...
    // Display inclination: proportional, capped so outer shells stay readable
    const iScale = Math.min(18, Math.abs(iDeg)) * (Math.abs(iDeg) > 20 ? 0.08 : 0.14);
    const st = keplerianXYZ(m.dist, e, iScale, node, peri, Mdeg);
    // Parent-local coordinates (x,z orbital plane; y out of plane)
    m.pos.set(st.x, st.z, st.y);
    m.mesh.position.copy(m.pos);

    // Tidal locking for large moons: same face toward parent
    if(m.name === 'Moon' || m.name === 'Io' || m.name === 'Europa' || m.name === 'Ganymede' ||
       m.name === 'Callisto' || m.name === 'Titan' || m.radius >= 0.07){
      // Face parent: angle of position vector in XZ
      const face = Math.atan2(m.pos.z, m.pos.x);
      m.mesh.rotation.y = face + (m.retrograde ? Math.PI : 0);
    } else {
      // Small irregulars: slow free rotation
      m.mesh.rotation.y += 0.0015 * Math.min(1, 30 / period);
    }

    // Keep orbit line geometry in sync for elliptical paths
    if(m.orbitLine && m.orbitLine.geometry && m._orbitNeedsUpdate !== false){
      // rebuild once when e is significant
      if(!m._orbitBuilt || Math.abs((m._orbitE||0) - e) > 0.002){
        const segsO = 96;
        const pts = [];
        for(let i=0;i<=segsO;i++){
          const Md = (i/segsO)*360;
          const s = keplerianXYZ(m.dist, e, iScale, node, peri, Md);
          pts.push(new THREE.Vector3(s.x, s.z, s.y));
        }
        m.orbitLine.geometry.dispose();
        m.orbitLine.geometry = new THREE.BufferGeometry().setFromPoints(pts);
        m._orbitBuilt = true;
        m._orbitE = e;
      }
    }
...
(function buildAsteroidRocks(){ try {
  ...
  const AW = 384, AH = 384; // higher-res regolith + crater normals
...
function updateDwarfLabels(){
  ...
  function overlaps(x,y){
    for(let i=0;i<placed.length;i++){
      if(Math.abs(placed[i].x-x)<90 && Math.abs(placed[i].y-y)<22) return true;
    }
    return false;
  }
  ...
  bodies.forEach(function(a){
    ...
    // Prefer live orbital position if mesh is in scene
    mesh.getWorldPosition(vec);
    if(!isFinite(vec.x) || !isFinite(vec.y) || !isFinite(vec.z)){
      // Fallback: place from AU + angle
      const r = displayDistanceFromAU(a.au || 2.5);
      const ang = (a.theta0 || 0);
      vec.set(r * Math.cos(ang), 0, r * Math.sin(ang));
    }
    const proj = vec.clone().project(camera);
    let x = (proj.x*0.5+0.5)*window.innerWidth;
    let y = (-proj.y*0.5+0.5)*window.innerHeight;
    // NDC z in three.js is in [-1,1]; behind camera if z>1 after project in older code — use w-check via z
    if(!(proj.z < 1 && x > -40 && x < window.innerWidth + 40 && y > -20 && y < window.innerHeight + 20)){
      a.labelEl.style.display='none'; return;
    }
    let tries=0;
    while(overlaps(x,y) && tries<14){ y += 16; tries++; }
    placed.push({x:x,y:y});
    a.labelEl.style.display='block';
    a.labelEl.style.left = Math.round(x) + 'px';
    a.labelEl.style.top = Math.round(y) + 'px';
    a.labelEl.style.zIndex = '14';
...
function selectPlanet(key){
  ...
  // Physical stats rows
  function showStat(rowId, valId, value, show){
    const row = document.getElementById(rowId);
    const el = document.getElementById(valId);
    if(row) row.style.display = (show === false || value == null || value === '') ? 'none' : '';
    if(el && value != null) el.textContent = value;
  }
  // Inclination from ELEMENTS when available
  let inclTxt = null;
  try {
    if(typeof ELEMENTS !== 'undefined' && ELEMENTS[p.key]){
      const el = ELEMENTS[p.key];
      const T0 = 0;
      const I = el.I[0] + el.I[1]*T0;
      inclTxt = Math.abs(I).toFixed(2) + '° to ecliptic';
    }
  } catch(e){}
  showStat('hud-incl-row', 'hud-incl', inclTxt, !!inclTxt);
  document.getElementById('hud-distance-label').textContent = 'Distance from Sun (mean)';
  document.getElementById('hud-ptype-label').textContent = 'Planet type';
  showStat('hud-gravity-row', 'hud-gravity', p.gravity != null ? (p.gravity + ' m/s²') : null, p.gravity != null);
  showStat('hud-temp-row', 'hud-temp', p.meanTempC != null ? (p.meanTempC + ' °C') : null, p.meanTempC != null);
  showStat('hud-density-row', 'hud-density', p.density != null ? (p.density + ' g/cm³') : null, p.density != null);
  showStat('hud-escape-row', 'hud-escape', p.escapeKmS != null ? (p.escapeKmS + ' km/s') : null, p.escapeKmS != null);
  // Rotation period + axial tilt (append into surface/mag rows when free)
  if(p.rotationHours != null){
    const hrs = p.rotationHours;
    const abs = Math.abs(hrs);
    let rotTxt;
    if(abs >= 48) rotTxt = (abs/24).toFixed(1) + ' days' + (hrs < 0 ? ' (retrograde)' : '');
    else rotTxt = abs.toFixed(2) + ' h' + (hrs < 0 ? ' (retrograde)' : '');
    // reuse surface row label when no surface text
    if(!p.surface){
      const lab = document.querySelector('#hud-surface-row .stat-label');
      if(lab) lab.textContent = 'Rotation period';
      showStat('hud-surface-row', 'hud-surface', rotTxt, true);
    }
  }
  if(p.axialTiltDeg != null && !p.magneticField){
    const lab = document.querySelector('#hud-mag-row .stat-label');
    if(lab) lab.textContent = 'Axial tilt';
    showStat('hud-mag-row', 'hud-mag', p.axialTiltDeg.toFixed(2) + '°', true);
  } else if(p.axialTiltDeg != null && p.magneticField){
    showStat('hud-mag-row', 'hud-mag', p.magneticField + ' · tilt ' + p.axialTiltDeg.toFixed(1) + '°', true);
  }
  showStat('hud-atmo-row', 'hud-atmo', p.atmosphere || null, !!p.atmosphere);
  showStat('hud-surface-row', 'hud-surface', p.surface || null, !!p.surface);
  showStat('hud-mag-row', 'hud-mag', p.magneticField || null, !!p.magneticField);

  const vis = document.getElementById('hud-visibility');
  if(vis) vis.textContent = p.visibility || '';
  const vb = document.getElementById('hud-vis-block'); if(vb) vb.style.display = '';
  const lb = document.getElementById('hud-live-block'); if(lb) lb.style.display = '';
...
function onPointerClick(ev){
  if(typeof dragMoved !== 'undefined' && dragMoved){ dragMoved=false; return; }
...
function focusMoon(m){
  ...
  const api = (typeof MOON_API !== 'undefined' && MOON_API.getLocal) ? MOON_API.getLocal(m.name) : null;
  const parentName = (m.parent || '').charAt(0).toUpperCase() + (m.parent || '').slice(1);

  document.getElementById('hud-type').textContent = 'Moon of ' + parentName;
  document.getElementById('hud-name').textContent = m.name;
  document.getElementById('hud-subtitle').textContent =
    (m.discovered ? ('Discovered ' + m.discovered + (m.discoverer ? ' · ' + m.discoverer : '')) : (m.periodLabel || 'Natural satellite'));

  document.getElementById('hud-diameter').textContent = m.diameterKm ? (Number(m.diameterKm).toLocaleString() + ' km') : '—';
  document.getElementById('hud-period').textContent = m.periodLabel || (m.periodDays ? (m.periodDays + ' days') : '—');

  const inclRow = document.getElementById('hud-incl-row');
  if(inclRow){
    inclRow.style.display = (m.iDeg != null) ? '' : 'none';
    const inclEl = document.getElementById('hud-incl');
    if(inclEl) inclEl.textContent = (m.iDeg != null) ? (Number(m.iDeg).toFixed(2) + '°') : '—';
  }

  document.getElementById('hud-distance-label').textContent = m.aKm ? 'Semi-major axis' : 'Orbit';
  if(m.aKm){
    document.getElementById('hud-distance').textContent = (m.aKm >= 1000 ? (m.aKm/1000).toFixed(0) + ' × 10³ km' : m.aKm + ' km') + ' from ' + parentName;
  } else {
    document.getElementById('hud-distance').textContent = 'Orbiting ' + parentName + (m.e != null ? (' · e≈' + Number(m.e).toFixed(3)) : '');
  }

  document.getElementById('hud-ptype-label').textContent = 'Body type';
  document.getElementById('hud-ptype').textContent = 'Natural satellite';

  // Sibling moons of the same planet — switch without leaving HUD
  const sibs = (typeof MOONS !== 'undefined')
    ? MOONS.filter(function(x){ return x.parent === m.parent && x.mesh; })
        .slice().sort(function(a,b){ return (a.dist||0)-(b.dist||0); })
    : [];
  const moonsRow = document.getElementById('hud-moons-row');
  if(moonsRow){
    moonsRow.style.display = '';
    const moonsVal = document.getElementById('hud-moons');
    if(moonsVal) moonsVal.textContent = String(sibs.length) + ' of ' + parentName;
  }
  const ml = document.getElementById('hud-moon-list');
  if(ml){
    ml.innerHTML = '';
    if(sibs.length){
      ml.style.display = 'block';
      const head = document.createElement('div');
      head.style.cssText = 'font-size:9px;letter-spacing:0.1em;text-transform:uppercase;color:var(--muted);margin:0 0 6px;width:100%;';
      head.textContent = 'Moons of ' + parentName + ' — click to switch';
      ml.appendChild(head);
      sibs.forEach(function(sib){
        const chip = document.createElement('button');
        chip.type = 'button';
        chip.className = 'moon-chip' + (sib === m || sib.name === m.name ? ' active' : '');
        chip.textContent = sib.name;
        chip.title = 'Focus on ' + sib.name;
        if(sib === m || sib.name === m.name){
          chip.style.borderColor = 'var(--brass)';
          chip.style.color = 'var(--brass)';
          chip.style.background = 'rgba(201,162,39,0.18)';
        }
        chip.addEventListener('click', function(ev){
          ev.preventDefault();
          ev.stopPropagation();
          if(sib !== m && typeof focusMoon === 'function') focusMoon(sib);
        });
        ml.appendChild(chip);
      });
    } else {
  ...
  // Gravity / temp from MOON_API_DATA when available
  function showRow(id, valId, val){
    const row = document.getElementById(id);
    const el = document.getElementById(valId);
    if(!row || !el) return;
    if(val == null || val === ''){ row.style.display = 'none'; return; }
    row.style.display = '';
    el.textContent = val;
  }
  const g = api && api.surfaceGravity != null ? api.surfaceGravity : (m.surfaceGravity != null ? m.surfaceGravity : null);
  showRow('hud-gravity-row', 'hud-gravity', g != null ? (g + ' m/s²') : null);
  const temp = api && api.meanTempC != null ? api.meanTempC : (m.meanTempC != null ? m.meanTempC : null);
  showRow('hud-temp-row', 'hud-temp', temp != null ? (temp + ' °C') : null);
  showRow('hud-density-row', 'hud-density', (api && api.albedo != null) ? ('Bond albedo ~' + api.albedo) : null);
  showRow('hud-escape-row', 'hud-escape', null);
  showRow('hud-atmo-row', 'hud-atmo', m.name === 'Titan' ? 'N₂-rich thick atmosphere' : (m.name === 'Moon' ? 'Exosphere only' : null));
  showRow('hud-surface-row', 'hud-surface', (api && api.textureHint) ? String(api.textureHint) : null);
  showRow('hud-mag-row', 'hud-mag', null);

  const vis = document.getElementById('hud-visibility');
  if(vis) vis.textContent = m.fact || (api && api.fact) || ('Natural satellite of ' + parentName + '.');
  const vb = document.getElementById('hud-vis-block'); if(vb) vb.style.display = '';
  const lb = document.getElementById('hud-live-block'); if(lb) lb.style.display = 'none';
...
if(todaybtn) todaybtn.addEventListener('click', function(){
  ...
  // Snap planet positions to now
  try {
    const Tnow = centuriesSinceJ2000(toJulianDate(simulatedDate));
    PLANETS.forEach(function(p){ if(typeof setPlanetPosition==='function') setPlanetPosition(p, Tnow); });
  } catch(e){}
});
...
// Inclination markers toggle
(function wireInclinationToggle(){
  const btn = document.getElementById('inclbtn');
  const legend = document.getElementById('incl-legend');
  if(!btn) return;
  function apply(){
    if(typeof inclVizGroup !== 'undefined' && inclVizGroup) inclVizGroup.visible = inclinationVisible;
    btn.textContent = inclinationVisible ? 'Inclination ON' : 'Inclination OFF';
    btn.classList.toggle('active', inclinationVisible);
    if(legend) legend.style.display = inclinationVisible ? 'block' : 'none';
  }
  apply();
  btn.addEventListener('click', function(){
    inclinationVisible = !inclinationVisible;
    apply();
  });
})();
...
window.addEventListener('keydown', function(e){
  ...
  if(e.key === ' ' || e.code === 'Space'){
    ...
    if(hud && hud.classList.contains('open')) hud.classList.remove('open');
    ...
    const ib = document.getElementById('inclbtn');
    if(ib) ib.click();
...
let isDragging=false, prevX=0, prevY=0, dragMoved=false, dragStartX=0, dragStartY=0;
...
function animate(now){
  ...
  if(playing){
    ...
    // Keep simulated-date readout in sync (UTC + local-friendly string)
    (function updateSimDateLabel(){
      ...
      el.textContent = y + '-' + mo + '-' + da + '  ' + hh + ':' + mm + ' UTC';
    })();
  ...
  updateDwarfLabels();
  updateBeltLabels();
  // Live distance / orbital readout while a planet is focused
  if(focusedPlanet && playing){
    try {
      const Tlive = centuriesSinceJ2000(toJulianDate(simulatedDate));
      const st = heliocentric(focusedPlanet.key, Tlive);
      const distEl = document.getElementById('hud-distance');
      const lab = document.getElementById('hud-distance-label');
      if(distEl && st && isFinite(st.r)){
        if(lab) lab.textContent = 'Distance from Sun (now)';
        distEl.textContent = st.r.toFixed(4) + ' AU';
      }
    } catch(e){}
  }
...
console.info('[Solar System Simulator] v2.4.0 — corrected scales · moon orbits · Mars/Jupiter/Saturn textures · HUD data.');
